...
#ifndef IRCAsyncSocketLoggingEnabled
#define IRCAsyncSocketLoggingEnabled 0
#endif

#if IRCAsyncSocketLoggingEnabled

// Logging Enabled - See log level below

// Logging uses the CocoaLumberjack framework (which is also IRC based).
...
#define LogAsync   YES
#define LogContext IRCAsyncSocketLoggingContext
...
#ifndef IRCAsyncSocketLogLevel
#define IRCAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = IRCAsyncSocketLogLevel;
...
NSString *const IRCAsyncSocketException = @"IRCAsyncSocketException";
NSString *const IRCAsyncSocketErrorDomain = @"IRCAsyncSocketErrorDomain";

NSString *const IRCAsyncSocketQueueName = @"IRCAsyncSocket";
NSString *const IRCAsyncSocketThreadName = @"IRCAsyncSocket-CFStream";

NSString *const IRCAsyncSocketManuallyEvaluateTrust = @"IRCAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const IRCAsyncSocketUseCFStreamForTLS = @"IRCAsyncSocketUseCFStreamForTLS";
#endif
NSString *const IRCAsyncSocketSSLPeerID = @"IRCAsyncSocketSSLPeerID";
NSString *const IRCAsyncSocketSSLProtocolVersionMin = @"IRCAsyncSocketSSLProtocolVersionMin";
NSString *const IRCAsyncSocketSSLProtocolVersionMax = @"IRCAsyncSocketSSLProtocolVersionMax";
NSString *const IRCAsyncSocketSSLSessionOptionFalseStart = @"IRCAsyncSocketSSLSessionOptionFalseStart";
NSString *const IRCAsyncSocketSSLSessionOptionSendOneByteRecord = @"IRCAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const IRCAsyncSocketSSLCipherSuites = @"IRCAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const IRCAsyncSocketSSLDiffieHellmanParameters = @"IRCAsyncSocketSSLDiffieHellmanParameters";
#endif

enum IRCAsyncSocketFlags
{
...
@interface IRCAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation IRCAsyncSocketPreBuffer
...
/**
 * The IRCAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface IRCAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(IRCAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation IRCAsyncReadPacket
...
/**
 * The IRCAsyncWritePacket encompasses the instructions for any given write.
**/
@interface IRCAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation IRCAsyncWritePacket
...
/**
 * The IRCAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface IRCAsyncSpecialPacket : NSObject
{
  @public
...
@implementation IRCAsyncSpecialPacket
...
@implementation IRCAsyncSocket
{
	...
	IRCAsyncReadPacket *currentRead;
	IRCAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	IRCAsyncSocketPreBuffer *preBuffer;
	...
	IRCAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([IRCAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[IRCAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak IRCAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong IRCAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create IRCAsyncSocket instance for accepted socket
			
			IRCAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"IRCAsyncSocketConnectTimeoutError",
	                                                     @"IRCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"IRCAsyncSocketReadMaxedOutError",
														 @"IRCAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"IRCAsyncSocketReadTimeoutError",
	                                                     @"IRCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"IRCAsyncSocketWriteTimeoutError",
	                                                     @"IRCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"IRCAsyncSocketClosedError",
	                                                     @"IRCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:IRCAsyncSocketErrorDomain code:IRCAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the IRCAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	IRCAsyncReadPacket *packet = [[IRCAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[IRCAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[IRCAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued IRCAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued IRCAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via IRCAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		IRCAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		IRCAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	IRCAsyncWritePacket *packet = [[IRCAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[IRCAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[IRCAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued IRCAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued IRCAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a IRCAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		IRCAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	IRCAsyncSpecialPacket *packet = [[IRCAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			IRCAsyncSpecialPacket *tlsPacket = (IRCAsyncSpecialPacket *)currentRead;
			NSDictionary *tlsSettings = tlsPacket->tlsSettings;
			
			NSNumber *value = [tlsSettings objectForKey:IRCAsyncSocketUseCFStreamForTLS];
...
{
	IRCAsyncSocket *asyncSocket = (__bridge IRCAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:IRCAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. IRCAsyncSocketSSLPeerID
	//  4. IRCAsyncSocketSSLProtocolVersionMin
	//  5. IRCAsyncSocketSSLProtocolVersionMax
	//  6. IRCAsyncSocketSSLSessionOptionFalseStart
	//  7. IRCAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. IRCAsyncSocketSSLCipherSuites
	//  9. IRCAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. IRCAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. IRCAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. IRCAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. IRCAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. IRCAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. IRCAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:IRCAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for IRCAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for IRCAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use IRCAsyncSocketSSLProtocolVersionMin & IRCAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[IRCAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"IRCAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[IRCAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[IRCAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("IRCAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:IRCAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(IRCAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(IRCAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	IRCAsyncSocket *asyncSocket = (__bridge IRCAsyncSocket *)pInfo;
