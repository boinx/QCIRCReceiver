...
@class IRCAsyncReadPacket;
@class IRCAsyncWritePacket;
@class IRCAsyncSocketPreBuffer;

extern NSString *const IRCAsyncSocketException;
extern NSString *const IRCAsyncSocketErrorDomain;

extern NSString *const IRCAsyncSocketQueueName;
extern NSString *const IRCAsyncSocketThreadName;

extern NSString *const IRCAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const IRCAsyncSocketUseCFStreamForTLS;
#endif
#define IRCAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define IRCAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define IRCAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const IRCAsyncSocketSSLPeerID;
extern NSString *const IRCAsyncSocketSSLProtocolVersionMin;
extern NSString *const IRCAsyncSocketSSLProtocolVersionMax;
extern NSString *const IRCAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const IRCAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const IRCAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const IRCAsyncSocketSSLDiffieHellmanParameters;
#endif

#define IRCAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, IRCAsyncSocketError) {
	IRCAsyncSocketNoError = 0,           // Never used
	IRCAsyncSocketBadConfigError,        // Invalid configuration
	IRCAsyncSocketBadParamError,         // Invalid parameter was passed
	IRCAsyncSocketConnectTimeoutError,   // A connect operation timed out
	IRCAsyncSocketReadTimeoutError,      // A read operation timed out
	IRCAsyncSocketWriteTimeoutError,     // A write operation timed out
	IRCAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	IRCAsyncSocketClosedError,           // The remote peer closed the connection
	IRCAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface IRCAsyncSocket : NSObject

/**
 * IRCAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, IRCAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means IRCAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means IRCAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, IRCAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, IRCAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of IRCAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a IRCAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a IRCAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that IRCAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - IRCAsyncSocketManuallyEvaluateTrust
 ...
 *     So IRCAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - IRCAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default IRCAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct IRCAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, IRCAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by IRCAsyncSocket,
 ...
 *
 * - IRCAsyncSocketSSLProtocolVersionMin
 * - IRCAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - IRCAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see IRCAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use IRCAsyncSocketSSLProtocolVersionMin & IRCAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * IRCAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the IRC API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(IRCAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(IRCAsyncSocket *)sock didAcceptNewSocket:(IRCAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful IRC API isn't notified properly in this case.
 * 
...
@protocol IRCAsyncSocketDelegate
@optional
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(IRCAsyncSocket *)sock didConnectToUrl:(NSURL *)url;
...
**/
- (void)socket:(IRCAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(IRCAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(IRCAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(IRCAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(IRCAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(IRCAsyncSocket *)sock;
 ...
 * 
 * Note: If the IRCAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socket:(IRCAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
